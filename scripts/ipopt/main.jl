...
function auglag(nlp, x0; iter_max=200, itout_max=1)
    ...
    aug = @time ExaOpt.AugLagEvaluator(nlp, x0; c₀=c0, scale=true)
    opt = () -> Ipopt.Optimizer(print_level=0, tol=1e-4)
    solution = @time ExaOpt.optimize!(algo, aug, x0; moi_optimizer=opt)
    return aug, solution
end
...
datafile = joinpath(dirname(@__FILE__), "..","..", "..", "ExaPF.jl", "data", "case118.m")
...
sol = auglag(slk, ExaOpt.initial(slk))
